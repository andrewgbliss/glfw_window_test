...
#include "OscillatingRectangle.h"
...
inline void NodeTypeMap::initializeMaps()
{
  ...
  nodeTypes["Sprite2D"] = [](const std::string &name, const Position2D &position, const Scale2D &scale, const Color &color)
  {
    return std::make_unique<Sprite2D>(name, position, scale);
  };
